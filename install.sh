...
echo ""
echo -e "${BLUE}==========================================${RESET}"
echo -e "${BLUE}        RAMRECON GLOBAL INSTALLER TOOL        ${RESET}"
echo -e "${BLUE}==========================================${RESET}"
echo ""
...
# --- Step 4: Copy RAMRecon to /opt ---
echo -e "${BLUE}[INFO] Copying RAMRecon source to $INSTALL_DIR...${RESET}"
mkdir -p "$INSTALL_DIR"
cp -r "$(pwd)/ramrecon" "$INSTALL_DIR/"
chmod -R 755 "$INSTALL_DIR"

# --- Step 5: Auto-create __main__.py if missing ---
if [ ! -f "$INSTALL_DIR/ramrecon/__main__.py" ]; then
    echo -e "${YELLOW}[INFO] Missing __main__.py detected. Creating automatically...${RESET}"
    cat <<'EOF' > "$INSTALL_DIR/ramrecon/__main__.py"
...
if [ "\$1" == "uninstall" ]; then
    ...
    PYTHONPATH=/opt/ramrecon python3 -m ramrecon "\$@"
fi
EOF
chmod +x "$BIN_PATH"
...
#!/bin/bash
# ==========================================
#   RAMRECON Uninstallation Script (Auto)
# ==========================================
APP_NAME="ramrecon"
INSTALL_DIR="/opt/$APP_NAME"
BIN_PATH="/usr/local/bin/$APP_NAME"
...
echo ""
echo -e "${BLUE}==========================================${RESET}"
echo -e "${BLUE}          RAMRECON UNINSTALLER TOOL          ${RESET}"
echo -e "${BLUE}==========================================${RESET}"
echo ""

if [ "$EUID" -ne 0 ]; then
    echo -e "${RED}[❌] Please run as root (sudo ramrecon uninstall)${RESET}"
    exit 1
fi

echo -e "${YELLOW}This will permanently remove RAMRecon.${RESET}"
...
rm -rf "$INSTALL_DIR" "$BIN_PATH"
echo -e "${GREEN}[✔] RAMRecon successfully removed.${RESET}"
echo ""
UNINSTALL
...
PYTHONPATH=/opt/ramrecon python3 -c "import ramrecon" >/dev/null 2>&1 && \
...
echo ""
echo -e "${BLUE}==========================================${RESET}"
echo -e "${GREEN}✅ RAMRECON INSTALLED SUCCESSFULLY (GLOBAL)${RESET}"
echo -e "${BLUE}==========================================${RESET}"
echo ""
echo -e "📦 Location:      $INSTALL_DIR"
echo -e "⚙️  Executable:    $BIN_PATH"
echo -e "📜 Requirements:  Installed from $(basename "$LOCAL_REQ")"
echo -e "🧹 Uninstall via: ${YELLOW}ramrecon uninstall${RESET}"
echo ""
echo -e "🚀 Launch RAMRecon globally using: ${YELLOW}ramrecon${RESET}"
echo ""
echo -e "${BLUE}==========================================${RESET}"
echo ""
